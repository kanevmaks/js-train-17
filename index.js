...
function replaceText(word, replacement, text) {
  ...
  const replacedText = text.replace(re, replacement);
  return replacedText;
...
function checkRegexFlags(regex) {
  const hasGlobalFlag = regex.flags.includes("g");
  const hasMultilineFlag = regex.flags.includes("m");
...
function replaceWordOccurrences(str, word, newWord) {
  const re = new RegExp(`\\b${word}\\b`, "g");
  const result = str.replace(re, newWord);
  return result;
...
function checkRegexMethods(regex) {
  ...
  return usedMethods; // Повертаємо масив використаних методів.
